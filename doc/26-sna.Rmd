...
Formal lässt sich dies als Wahrscheinlichkeit  auffassen, dass eine Kante $(u,v)$ und deren Gegenstück $(v,u)$ existiert (vgl. [@Wasserman1994], S. 515):
\begin{equation}
P(u \to v \wedge v \to u) = P(u \to v)P(v \to u|u\to v)
(\#eq:probmut)
\end{equation}

Die Idee von Katz und Powell besteht darin, die bedingte Wahrscheinlichkeit im letzten Term von \@ref(eq:probmut) mittels eines Maßes $\rho_{KP}$ aufzuschlüsseln (vgl. [@Wasserman1994]):
...
Die Wahrscheinlichkeit einer zu $(u,v)$ erwiderten Kante setzt sich also zusammen aus der Wahrscheinlichkeit der Gegenkante, plus einem Anteil $\rho_{KP}$ der Wahrscheinlichkeit, dass diese Gegenkante nicht existiert.
Die Formulierung der Wahrscheinlichkeit in \@ref(eq:rhomut) bringt einige interessante Eigenschaften von $\rho_{KP}$ mit:
0:
1:
-1:
