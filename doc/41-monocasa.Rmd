...
Solche Löcher werden in der urpsrünglichen Definition von Katz und Powell [@Katz1955] nicht erwähnt und stellen ein Detail der Implementierung dar, um auch Spezialfälle sinnvoll im Diagramm festzuhalten.
...
In Abbildung \@ref(fig:monocasa-topical-subgraph) ist für jeden der beiden Graphen $G$ und $G'$ die Zahl der Knoten $|G|$ und $|G'|$ sowie deren Differenv $|G|- |G'|$ im zeitlichen Verlauf angetragen; $|G|$ wird auch als *Ordnung* des Graphen bezeichnet.
...
(ref:monocasa-topical-subgraph) Ordnung des gesamten Graphen $G$ und des thematischen Teilgraphen $G'$,sowie deren Differenz $|G| - |G|'$. Die angetragenen Regressionslinien verdeutlichen das unterschiedliche Wachstum der Größen.
...
Es sei darauf hingewiesen, dass dieser Ansatz nicht nur Aussagen über Ego zulässt, sondern auch über die Alteri, da der Indsx $\rho_{KP}$ stets das ganze Netzwerk in Betracht zieht.
...
Um dies zu prüfen, werden für den gesamten Graph $G$ und den "monothematischen" Teilgraph $G'$ die Indexe berechnet, sowie die Differenz beiden Maße, $\rho_{kp}(G')-\rho_{KP}(G)$.
