...
Um einen Überblick zu erhalten, in welchen Topics der Nutzer aktiv ist, stellen wir die Themenhistorie über den vollen Untersuchungszeitraum dar.
Alle Kommentare werden über das Subreddit, in dem sie erstellt wurden, einem Topic zugeordnet.
In Abbildung \@ref(fig:monocasa-area-chart-comms-full) ist zu erkennen, dass auf Topic 239 ein Großteil aller Kommentare von monocasa entfallen, gefolgt von Topic 210.
...
Bei der Analyse des Topic-Verlaufs fallen mehrere Entwicklungen auf.
Zum einen beginnt ab ca. Januar 2013 der Anteil an Topic 210 (Politik) zu wachsen, gleichzeitig nimmt der Anteil von 239 (Technik/IT) ab, im Oktober 2013 sinkt der Anteil sogar auf 0.
Anfang 2015 erreicht dieses Topic jedoch wieder eien hohen Anteil, fällt danach aber zusehends ab.

Für die Alteri lässt sich ein ähnlicher Topic-Verlauf bestimmen, indem pro Monat für jeden von ihnen das Topic bestimmt wird, wo er die meisten Kommentare verfasst; so wird jeder dieser Nutzer auf ein einzelnes Topic festgelegt, das ihn charakterisiert.
Damit lässt sich bestimmen, welchen Anteil ein Topic unter den Alteri ausmacht.
Abbildung \@ref(fig:monocasa-area-chart-alters-full) lässt ebenfalls erkennen, dass ab etwa Mitte 2014 der Anteil des Politik-Topics 210 zu wachsen beginnt, und dass das Technik-Topic 239 gegen Ende 2013/Anfang 2014 absinkt.
...
Da diese Phase scheinbar von einiger Dynamik geprägt ist, fokussieren wir die weitere Analyse auf den Zeitraum von Dezember 2012 bis Dezember 2017, also genau fünf Jahre; dieser Fokus wird durch den leicht schattierten Bereich in den beiden letzten Abbildungen hervorgehoben.
Alle weiteren zeitlichen Verläufe beziehen sich auf diesen neuen Zeitraum der Untersuchung.
...
```{r monocasa-area-chart-alters-full, fig.cap="(ref:monocasa-area-chart-alters-full)",cache=FALSE}
...
#### Kommentarverteilungen
Bislang wurden relative Anteile eines Topics an allen Kommentaren dargestellt.
...
```{r monocasa-num-posts, cache=FALSE}
...
```{r monocasa-interestingness-zoom}
df %>%
  group_by(topic) %>%
