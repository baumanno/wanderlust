...
### monocasa
\FloatBarrier
Der Nutzer mit dem Namen "monocasa" ist im betrachteten Ausschnitt aus dem Datensatz in 119 Monaten enthalten und hat insgesamt 7996 Kommentare erstellt, was im Mittel `r fmt(round(7996 / 119, 2))` Kommentaren pro Monat entspricht.
...
Wie bereits weiter oben festgestellt, handelt es sich bei Topic 239 um technisch orientierte Inhalte, bei 210 um Politik mit Schwerpunkt auf US-amerikanischen Themen; Topic 235 wurde als Reddit-selbstreferentiell identifiziert und wird von näherer Betrachtung ausgeschlossen.
...
```{r monocasa-topic-distribution, fig.cap="monocasa-topic-distribution"}
...
```{r monocasa-num-posts, fig.cap="(ref:monocasa-num-posts)", include=TRUE}
...
(ref:monocasa-prob-corr) Die beiden Anteile an Topic-Zugehörigkeit, gegeneinander aufgetragen. Die x-Achse zeigt den relativen Anteil Kommentare, die Ego in einem Topic verfasst, die y-Achse den Anteil Alteri, der sich diesem Topic hauptsächlich widmet. Spearman's rho zeigt die Stärke der Korrelation ($^{*}p\le0.05$, $^{**}p\le0.01$, $^{***}p\le0.001$).
...
Da beide Faktoren relative Anteile darstellen, ist auch der Index daraus dimensionslos.

(ref:monocasa-interestingness) Attraktivität angetragen über den zeitlichen Verlauf. Die gepunktete Linie kennzeichnet den Zeitpunkt des Verlassens von Topic 239. In dieser Darstellung enthalten sind alle Topics, deren Attraktivität in Summer echt größer 0 ist.
...
Um einen ersten Eindruck von der Größe und Struktur der Kommunikationsbeziehungen des Nutzers zu erhalten, zeigt Abbildung \@ref(fig:monocasa-network-structure) die Anzahl der Knoten und Kanten im sozialen Graph des Nutzers, *über den gesamten Beobachtungszeitraum*.
...
```{r monocasa-katz-powell, fig.caption="(ref:monocasa-katz-powell)"}
...
Wie Abbildung \@ref(fig:monocasa-katz-powell) jedoch zeigt, steigt der Indexwert zu Beginn langsam an und erreicht dann im ersten Drittel sein vorläufiges Maximum.
