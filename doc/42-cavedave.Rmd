### cavedave
Der Nutzer mit dem Namen "cavedave" ist im betrachteten Ausschnitt aus dem Datensatz in 119 Monaten enthalten und hat insgesamt 7996 Kommentare erstellt, was im Mittel `r fmt(round(7996 / 119, 2))` Kommentaren pro Monat entspricht.

```{r cavedave-setup, cache=FALSE}
user <- "cavedave"
...
In Abbildung \@ref(fig:cavedave-topic-distribution) ist zu erkennen, dass der Nutzer hauptsächlich in drei Topics aktiv ist: 210, 239 und 235.
Auch der zeitliche Verlauf der Topics in Abbildung \@ref(fig:cavedave-area-chart-comms-full) lässt dies erkennen.
...
```{r cavedave-topic-distribution, fig.cap="cavedave-topic-distribution",cache=FALSE}
...
Der Untersuchungszeitraum wird auf die Zeit zwei Jahre vor und zwei Jahre nach dem Verlassen im Oktober 2013 festgelegt; dies ist durch die beiden gepunkteten Linien sowie die grau hinterlegte Fläche in Abb.\ \@ref(fig:cavedave-topic-distribution) angedeutet.
...
Auch Abbildung \@ref(fig:cavedave-area-chart-alters-full) lässt erkennen, dass in dem gewählten Zeitraum der Anteil der Alteri, die sich ebenfalls an diesem Topic beteiligen, auf 0 abfällt.
...
```
(ref:cavedave-area-chart-alters-full) Topic-Verteilung der Alteri im lokalen Netzwerk. Dargestellt sind die relativen Anteile eines Topics an allen Kommentaren, die Ego in einem gegebenen Monat erstellt.
...
(ref:cavedave-num-posts) Kommentare pro Monat pro Topic. Die gepunktete Linie zeigt das Datum des "Verlassens" von Topic 239.
```{r cavedave-num-posts, fig.cap="(ref:cavedave-num-posts)", include=TRUE}
...
(ref:cavedave-prob-corr) Die beiden Anteile an Topic-Zugehörigkeit, gegeneinander aufgetragen. Entlang der x-Achse ist der relative Anteil an Kommentaren angetragen, die Ego in einem Topic verfasst hat; entlang der y-Achse ist der relative Anteil Alteri angetragen, der einem Topic zugeordnet wurde. Die blaue Linie fittet ein lineares Modell auf den Daten.
...
(ref:cavedave-interestingness) Attraktivität angetragen über den zeitlichen Verlauf. Die gepunktete Linie kennzeichnet den Zeitpunkt des Verlassens von Topic 239. In dieser Darstellung enthalten sind alle Topics, deren Attraktivität in Summer echt größer 0 ist.
```{r cavedave-interestingness, fig.cpa="(ref:cavedave-interestingness)"}
...
Abbildung \@ref(fig:cavedave-interestingness) zeigt die zeitliche Entwicklung dieser Messgröße.
...
Abbildung\ \@ref(fig:cavedave-total-posts) zeigt den zeitlichen Verlauf erstellter Kommentare je Topic, sowie deren Summe.
...
(ref:cavedave-total-posts) Anzahl Kommentare je Topic über die Zeit aufgetragen, sowie die Summe aller Kommentare als rot gestrichelte Linie.
...
Um einen ersten Eindruck von der Größe und Struktur der Kommunikationsbeziehungen des Nutzers zu erhalten, zeigt Abbildung \@ref(fig:cavedave-network-structure) die Anzahl der Knoten und Kanten im sozialen Graph des Nutzers, *über den gesamten Beobachtungszeitraum*.
...
(ref:cavedave-network-structure) Zahl der Knoten sowie Kanten des lokalen sozialen Netzwerks, angetragen über die Zeit. Wie zuvor auch kennzeichnet die gepunktete Linie den Austritt aus Topic 239.
...
```{r cavedave-katz-powell, fig.caption="(ref:cavedave-katz-powell)"}
...
Wie Abbildung \@ref(fig:cavedave-katz-powell) jedoch zeigt, steigt der Indexwert zu Beginn langsam an und erreicht dann im ersten Drittel sein vorläufiges Maximum.
...
In Abbildung \@ref(fig:cavedave-topical-subgraph) ist für jeden der beiden Graphen $G$ und $G'$ die Zahl der Knoten $|G|$ und $|G'|$ sowie deren Differenz $|G|- |G'|$ im zeitlichen Verlauf angetragen; $|G|$ wird auch als *Ordnung* des Graphen bezeichnet.
...
(ref:cavedave-topical-subgraph) Ordnung des gesamten Graphen $G$ und des thematischen Teilgraphen $G'$,sowie deren Differenz $|G| - |G|'$. Die angetragenen Regressionsgeraden verdeutlichen das unterschiedliche Wachstum der Größen.
...
Abbildung \@ref(fig:cavedave-topical-subgraph-reciprocity) bestätigt die Vermutung in Teilen.
...
Abbildung \@ref(fig:cavedave-subgraph-per-topic) zeigt diese beiden Maße für jedes Topic, in Abbildung \@ref(fig:cavedave-subgraph-one-topic) wird speziell Topic 239 in den Fokus gerückt.
...
(ref:cavedave-subgraph-per-topic) Verteilung der Knoten und Kanten in den thematischen Teilgraphen.
```{r cavedave-subgraph-per-topic, fig.cap="(ref:cavedave-subgraph-per-topic)"}
graph_single_topic_subgraph %>% 
  mutate(
...
(ref:cavedave-subgraph-one-topic) Verteilung der Knoten und Kanten im thematischen Teilgraphen von Topic 239.
```{r cavedave-subgraph-one-topic, fig.cap="(ref:cavedave-subgraph-one-topic)"}
graph_single_topic_subgraph %>% 
  filter(topic == 239) %>% 
  mutate(
    vs = vvcount(topic_graph) - 1,
...
Die Verteilung der Kanten in Abbildung \@ref(fig:cavedave-subgraph-one-topic) weist in den Monaten vor dem Verlassen im Oktober 2013 vergleichsweise viele Kanten zu wenigen Knoten auf.
