...
```{r cavedave-total-posts, fig.cap="(ref:cavedave-total-posts)"}
...
Alle Alteri der monatlich erstellten Ego-Netzwerke werden auf das Topic reduziert, in dem sie zu diesem Zeitpunkt die meisten Kommentare verfasst haben. Die Darstellung zegt den Anteil, den die Nutzer eines Topics am lokalen Netzwerk von cavedave haben.  
Auffällig ist, dass sich Topic 219 nur wenige Alteri widmen, obwohl angänglich ein großer Teil von Egos Kommunikation darauf abfällt.
...
Die Scatterplots in Abbildung \@ref(fig:cavedave-prob-cor) zeigen ähnliche Ergebnisse für cavedave.
Da die berechneten Topic-Anteile nach Shapiro-Wilk nicht normalverteilt sind, wurde zur Bestimmung der Stärke der Korrelation Spearman's rho verwendet; mit Ausnahme der Topics 122 und 89 sind alle Korrelationen statistisch signifikant, für Korrelationen mit $\textit{r} = 1$ liegen jedoch in diesem Fall zu wenig Datenpunkte größer Null vor.
Bei Topic 219
...
Auch hier ersichtlich, dass der amrkierte Zeitraum von hoher Dynamik geprägt ist.
...
(ref:cavedave-subgraph-rec-all) Katz-Powell-Index im zeitlichen Verlauf, je Topic.
```{r cavedave-subgraph-rec-all, fig.cap="(ref:cavedave-subgraph-rec-all)"}
subgraph_reciprocity_all(graph_single_topic_subgraph)
```
...
Für Topic 239, in dem cavedave bis Mitte 2011 noch aktiv war, zeigt sich ein Abwärtstrend bei reziproken Beziehungen.
Bei Topic 219 fällt auf, dass der Wert des Index einige Male ins Negative fällt, bevor der Nutzer anfängt, hier seine Aktivität zurückzufahren.
...
die Korrelation des Katz-Powell-Index und des Anteils an Kommentaren fällt indes durchweg eher schwach aus, wie Abbildung \@ref(fig:cavedave-reciprocity-special-topics)
 verdeutlicht.

(ref:cavedave-reciprocity-special-topics) Katz-Powell-Index für drei ausgewählte Topics. Spearman's rho zeigt die Stärke der Korrelation ($^{*}p\le0.05$, $^{**}p\le0.01$, $^{***}p\le0.001$)
...
coeffs <- foo %>%
  ...
  filter(n() > 2) %>% 
      summarise(
        r = cor.test(prop_ego, kp, method = "spearman")$estimate,
        p = cor.test(prop_ego, kp, method = "spearman")$p.value
      )
...
toical_subgraph_order(graph_topical_subgraph)
...
"Monothematisch" Teilgraphen sind hier solche, bei denen auch Ego auf das Topic festgelegt wird, in dem er am aktivsten ist^[Für Ego wurden zuvor die fünf Topics betrachtet, in denen er in einem gegebenen Monat aktiv war.].  
...
Dabei fällt auf, dass die Differenz nahezu konstant bleibt und sowohl der Graph als auch der thematische Teilgraph annhähernd gleich stark wachsen.
...
Hier sind zwei Beobachtungen von besonderem Interesse; erstens nimmt die Differenz des Index zumeist negative Werte an, im Gesamtgraph werden Kanten also eher erwidert; und zweitens existiert zwischen Januar ein Loch in der Kurve des Teilgraphen und der Differenz.
Kann $\rho_{KP}$ nicht sinnvoll berechent werden, weil etwa der Graph keine Kanten enthält, gibt die im Rahmen dieser Arbeit erstellte Implementierung einen speziellen Wert^[In R signalisiert der Wert `NA` meist fehlende Daten.] zurück, der in der graphischen Darstellung ein Loch verursacht.
Der monothematische Teilgraph ist im Zeitraum zwischen Januar und August 2013 also leer; mit anderen Worten, es gibt keine Alteri, die in dieser Zeit hauptsächlich in dem Topic aktiv sind, in dem auch Ego aktiv ist.
Es ist diese Phase auch, die vom Umbruch zwischen Topic 219 und 235 gekennzeichnet ist.
