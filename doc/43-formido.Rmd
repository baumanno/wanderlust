### Formido - Daten und Definitionen

```{r formido-setup}
user <- "formido"
date_range_full_start <- "2007-01-01"
date_range_full_end <- "2018-02-01"
date_range_cut_start <- "2015-01-01"
date_range_cut_end <- "2017-09-01"
...
(ref:formido-area-chart-comms-full) Zuordnung von Egos Kommentaren zu Topics. Dargestellt sind die relativen Anteile eines Topics an allen Kommentaren, die Ego in einem gegebenen Monat erstellt.
...
```{r formido-area-chart-alters-full}
...
```{r formido-area-chart-comms-zoom}
...
```{r formido-area-chart-alters-zoom}
...
```{r formido-interestingness-zoom}
df %>%
  group_by(topic) %>%
