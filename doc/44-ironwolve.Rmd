### IronWolve - Daten und Definitionen

```{r ironwolve-setup}
user <- "IronWolve"
date_range_full_start <- "2007-01-01"
date_range_full_end <- "2018-02-01"
date_range_cut_start <- "2007-01-01"
date_range_cut_end <- "2018-02-01"
...
```{r ironwolve-topic-distribution}
ego_proportions_full %>%
  ggplot(mapping = aes(
    x = factor(topic),
    y = num_posts,
...
(ref:ironwolve-area-chart-comms-full) Zuordnung von Egos Kommentaren zu Topics. Dargestellt sind die relativen Anteile eines Topics an allen Kommentaren, die Ego in einem gegebenen Monat erstellt.
```{r ironwolve-area-chart-comms-full, fig.cap="(ref:ironwolve-area-chart-comms-full)"}
area_plot_posts(ego_proportions_full, NA, NA)
```

```{r ironwolve-area-chart-alters-full}
area_plot_alters(alters_proportions_full, NA, NA)
...
```{r ironwolve-area-chart-comms-zoom}
...
```{r ironwolve-area-chart-alters-zoom}
...
```{r ironwolve-interestingness-zoom}
df %>%
  group_by(topic) %>%
...
```{r ironwolve-single-topic-subgraph}
...
```{r ironwolve-subgraph-per-topic}
graph_single_topic_subgraph %>% 
  mutate(
...
```{r ironwolve-subgraph-one-topic}
graph_single_topic_subgraph %>% 
  filter(topic == 210) %>% 
  mutate(
    vs = vvcount(topic_graph) - 1,
...
```{r ironwolve-subgraph-reciprocity, cache = FALSE}
...
```{r ironwolve-degree-distribution}
graph_single_topic_subgraph %>%
  mutate(
