...
ego_proportions_full %>%
  ...
  geom_point(alpha = .1) +
  geom_smooth(span = .4) + facet_wrap(~ topic)
...
"verlässt" das Topic. Dies ist insofern bemerkenswert, als dass noch im Dezember 2010 dieses Topic das globale Maximum erreicht; `r ego_proportions_full[which.max(ego_proportions_full$prop), ]$prop * 100`% aller Posts des Users entfallen auf dieses Topic.
...
graph_data %>%
  ...
  geom_line(mapping = aes(y = katz_powell, colour = "Katz-Powell")) +
  geom_line(mapping = aes(y = centered_reciprocity, colour = "Indegree/Outdegree")) +
  geom_smooth(mapping = aes(y = katz_powell, colour = "Katz-Powell")) +
  geom_smooth(mapping = aes(y = centered_reciprocity, colour = "Indegree/Outdegree")) +
  scale_x_date(
    date_breaks = "3 months",
    date_minor_breaks = "1 month",
    date_labels = "%Y-%m"
  ) +
  scale_y_continuous(breaks = seq(-0.5, 1.5, by = 0.1), minor_breaks = seq(-0.5, 1.5, by = .05))
```
...
graph_single_topic_subgraph %>%
  mutate(
    kp = katz_powell_mutuality(topic_graph)
  ) %>%
  select(-graph) %>%
...
graph_single_topic_subgraph %>%
  mutate(
    ...
    bar = topic_graph_degree_ratio * prop
    ) %>%
  select(-num_posts,
         -graph) %>% 
  ggplot(mapping = aes(x = date, y = bar, colour = topic)) +
  geom_line() +
  facet_wrap(~topic)
```
