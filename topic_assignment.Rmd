...
df <- tibble(sub = subreddits, topic = factor(topics)) %>% 
  count(topic) %>% 
  mutate(rel = n/sum(n))
...
df %>%
  ...
  ggplot() + 
  geom_bar(mapping = aes(x = topic, y = rel, fill = highlight), stat = "identity") +
...
sind knapp 5% aller betrachteten Subreddits Topic Nummer 210 zugeordnet.
